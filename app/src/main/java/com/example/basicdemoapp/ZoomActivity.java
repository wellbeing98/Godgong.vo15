...
//        // 해당 게시물의 데이터 불러오기
        ...
        final String key = test.getString("key","DEFAULT");
        ...
        ValueEventListener postListener = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                PostZoom post = snapshot.getValue(PostZoom.class);
                writer = post.getWriterId();
                String date = post.getDate();
                String title = post.getTitle_et();
                String body = post.getContent_et();
                zoomId = post.getZoomId();
                zoomPwd = post.getZoomPwd();
                email = post.getEmailId();
                meetingno = post.getRoomNum();
                meetingpwd = post.getRoomPwd();
                ...
                if(pathReference == null){
                    Toast.makeText(ZoomActivity.this, "저장소에 사진이 없습니다.", Toast.LENGTH_SHORT).show();}
                else{
                    Toast.makeText(ZoomActivity.this, "저장소에 사진이 있습니다.", Toast.LENGTH_SHORT).show();


                    StorageReference submitProfile = storageReference.child("images/"+email);


                    Glide.with(ZoomActivity.this /* context */)
                            .load(submitProfile)
                            .into(im_project);
                }
            }
        ...
        mDatabase.child("studyposts").child(key).addListenerForSingleValueEvent(postListener);
        ...
        ValueEventListener registerListener = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot commentSnapshot : snapshot.getChildren()) {
                    ...
                    LayoutInflater layoutInflater = LayoutInflater.from(ZoomActivity.this);
...
//                    Toast.makeText(DetailActivity.this, com.getId(), Toast.LENGTH_SHORT).show();
        ...
        mDatabase.child("registerzoom").child(key).addValueEventListener(registerListener);
...
//
                ...
                idkey = mDatabase.child("registerzoom").child(key).push().getKey();



                mDatabase.child("registerzoom").child(key).child(idkey).setValue(userId);
...
//                    imm.hideSoftInputFromWindow(comment_et.getWindowToken(), 0);
                ...
                // 토스트메시지 출력
                Toast.makeText(ZoomActivity.this, "프로젝트에 참여되었습니다.", Toast.LENGTH_SHORT).show();
...
//            }
        ...
        cancel_button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                DatabaseReference dataref = mDatabase.child("registerzoom").child(key).child(idkey);
                dataref.removeValue();

                Toast.makeText(ZoomActivity.this, "프로젝트에 참여를 취소하였습니다.", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        chat_room.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Intent intent = new Intent(ZoomActivity.this, ChatActivity.class);
...
//                        EditText nameInput = dialog.findViewById(R.id.meeting_no_layout);
...
//                        EditText passwordInput = dialog.findViewById(R.id.password_input);
                        if (this.userid != null && this.meetingno != null &&  this.meetingpwd != null ) {
                            ...
                            if (meetingNumber.trim().length() > 0 && password.trim().length() > 0) {
                                joinMeeting(ZoomActivity.this, Name, meetingNumber, password);
                            }
                        }
    }
    public void joinMeeting(Context context, String Name, String meetingNumber, String password) {
        ...
        params.displayName = Name; // TODO: Enter your name
